...
int http::get_line(char* buf)   //按行读取请求报头
{
	char ch='\0';
	int i=0;
	ssize_t ret=0;
...
void http::echo_error(int err_code)    //错误处理
{
	switch(err_code)
	{
	case 403:
...
int http::echo_www(const char * path,size_t s)  //处理非CGI的请求
{
...
int http::excu_cgi(const char* method,\
...
int http::handler_msg()       //浏览器请求处理函数
{
	char buf[SIZE];
	int count=get_line(buf);
	int ret=0;
	char method[32];
	char url[SIZE];
	...
	{
		ret=excu_cgi(method,path,query_string);           //处理CGI模式的请求
	}
	else
	{
		clear_header();
		ret=echo_www(path,st.st_size);  //如果是GET方法，而且没有参数，请求的也不是可执行程序，则直接返回资源	
	}
